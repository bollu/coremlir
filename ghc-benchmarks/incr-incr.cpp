...
// 1. Outline List * since it's being forced
// 2. Explode data structure into 3 arguments.
namespace f2 {

List *incr(Thunk<List *> xst);

List *incrv_explode(ListTy ty, Thunk<SimpleInt> yt, Thunk<List *> yst) {
    // switch (xsv->ty) {
    switch (ty) {
        case ListTy::Nil: {
            return new ListNil();
...
List *incrv(List *xsv) {
    switch (xsv->ty) {
        case ListTy::Nil: {
            return incrv_explode(xsv->ty, undefThunk<SimpleInt>(), undefThunk<List *>());
        }
        case ListTy::Cons: {
            ListCons *xsv_cons = (ListCons *)(xsv);
            return incrv_explode(xsv->ty, xsv_cons->yt, xsv_cons->yst);
        }
    };
}
List *incr(Thunk<List *> xst) {
